...
因此，需要考察PMT批量测试平台的长期稳定性，以确认我们的测试结果真实可靠。
PMT批量测试平台具有两个固定不动的监控PMT，每个Run中它们都经历了与被测PMT一样的测试流程，从它们的测试结果中可以提取出平台的性能参数。

我们首先考察了LED光源的长期稳定性，结果如图\ref{fig:pmt_test:led_stability}所示。
可以看到在没有
\begin{figure}[htbp]
	\centering
