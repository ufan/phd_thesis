...
另外，PSD输出信号的动态范围直接决定于单元条端头PMT接收到的光子数，因此最终的动态范围还会受到光在单元条中传输导致的衰减效应的影响。
本节结合这些因素，对PSD的动态范围做了一个合理的估计，该估算结果被用于指导PSD探测单元模块大动态范围读出方案的设计。

在高能粒子探测器领域，设计人员经常使用最小电离粒子(Minimum Ionizing Particle，简称MIP)来对探测器进行测试和刻度。
对于PSD覆盖的相对论能区，所有的被测粒子（质子，重离子，电子）的能量都在Beth-Block曲线的费米坪区内，它们的电离能损与MIP相差不大，因此它们也都可以被称为MIP粒子。
我们将单电荷的最小电离粒子垂直穿过PSD塑闪单元条中心时端头PMT接收到的光输出量定义为\SI{1}{MIP}，并以它为单位来表述PSD的动态范围需求。
由上节的结果可以看到，不同入射重离子在PSD塑闪条中的光产额最大相差270倍。由于所有单电荷的
